...
//	int max=0;
	while (!queue.isEmpty()) {
		...
		if (node.fang>4||node.counts==d-1)
			{queue.pop();	
			
			continue;}
		
//		System.out.print(node.statestring+"    ");
//		System.out.print(node.fang+"  ");
//		System.out.println(node.counts);
//		
//		if(max<node.counts) max=node.counts;
		// 若该节点成为目标状态,输出所有父节点
...
//			dataField3.setText(dataField3.getText()+"   最长搜索链长"+max);
			printResult(node);
